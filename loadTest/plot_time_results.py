...
def parse_time_results(filename, max_lines=None):
    with open(filename, 'r') as file:
        ...
        times = [int(line.split('Time Taken: ')[1].split(' ms')[0]) for line in lines]
    return times

# Read and parse the data from both files, limiting to the first 50 lines
max_lines = 50
times_no_cache = parse_time_results('time_results.txt', max_lines)
# times_with_cache = parse_time_results('time_results_with_cache.txt', max_lines)
